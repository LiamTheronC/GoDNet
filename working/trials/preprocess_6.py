#This version of preprocess is based on version 5, picks out those
#scenarios with a focus on pedestrains.
...
class Waymo_Motion_Preprocess_6:
    ...
    def get_obj_states(self, index: int) -> dict:
        ...
        Returns:
            data: a dict with dict_keys(['objects_of_interest', 
                                        'time_stamps', 
                                        'current_time_index', 
                                        'scenario_id', 
                                        'sdc_index', 
        ...
        all_tracks['id'] = obj_id
        all_tracks['type'] = obj_type
        all_tracks['traj_xyz'] = obj_traj
        all_tracks['velocity_xy_heading'] = obj_velocity_heading
        all_tracks['shape'] = obj_shape
        all_tracks['valid_mask'] = obj_valid
        all_tracks['sdc_index'] = scen['sdcTrackIndex']

        if 'objectsOfInterest' in scen.keys():
            all_tracks['objects_of_interest'] = scen['objectsOfInterest']
        else:
            all_tracks['objects_of_interest'] = None

        track_to_predict = dict()

        track_to_predict['index'] = ttp_indx
        track_to_predict['id'] = np.array(obj_id)[ttp_indx]
        track_to_predict['type'] = np.array(obj_type)[ttp_indx]

        data['all_tracks'] = all_tracks
        data['track_to_predict'] = track_to_predict
    ...
    def get_obj_feats(self, data: dict) -> dict:
        ...
        trajs = data['all_tracks']['trajs_xyz']
        sdc_index = data['all_tracks']['sdc_index']
        time_index = data['current_time_index']

        orig = trajs[sdc_index][time_index]
        pre_orig = trajs[sdc_index][time_index-1]
        ...
        feats, ctrs, gt_preds, has_preds, engage_id = [], [], [], [], []
        gt2_preds = []
...
def get_nodes(engage_lanes,config):
    ...
    for id in lane_ids:
        ...
        ctrln = lane['polyline']
        ctrs.append(np.asarray((ctrln[:-1] + ctrln[1:]) / 2.0, np.float32)[::df])
        feats.append(np.asarray(ctrln[1:] - ctrln[:-1], np.float32)[::df])
