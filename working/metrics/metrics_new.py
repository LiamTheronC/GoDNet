...
class Postprocess():
    ...
    def append(self, metrics, loss, outputs, data):
        ...
        minDist_fde, min_idcs = get_minFDE(outputs['reg'],
                            data,
                            self.config["num_preds"],
                            self.config["num_mods"])

        
        minDist_ade = get_minADE(outputs['reg'],
                            data,
                            self.config["num_preds"],
                            self.config["num_mods"])
        
        minDist_fdeT,_ = get_minFDE(outputs['reg'],
                            data,
                            self.config["num_preds"],
                            self.config["num_mods"],
                            True)
        ...
        minDist_adeT = get_minADE(outputs['reg'],
                            data,
                            self.config["num_preds"],
                            self.config["num_mods"],
                            True)
