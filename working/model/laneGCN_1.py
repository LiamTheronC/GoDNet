...
def mark_gather(marks: List[Tensor]) -> Tuple[Tensor, List[Tensor]]:
    """
    get mark_idcs
    
    """

    batch_size = len(marks)
    num_marks = [len(x) for x in marks]

    marks = torch.cat(marks, 0).to(torch.float32)

    mark_idcs = []
    count = 0
    for i in range(batch_size):
        idcs = torch.arange(count, count + num_marks[i]).to(marks.device)
        mark_idcs.append(idcs)
        count += num_marks[i]

    return marks, mark_idcs
...
class Mr2Mr(nn.Module):
    """
    self-attention for markings 
    """

    def __init__(self, config) -> None:
        super(Mr2Mr, self).__init__()
        ng = 1
        norm = "GN"
        dropout=0.1
        n_in = 3
        n_hidden = config['n_mark']

        self.input = nn.Sequential(
                    nn.Linear(n_in, n_hidden),
                    nn.ReLU(inplace=True),
                    Linear(n_hidden, n_hidden, norm=norm, ng=ng, act=False),
                )
        ...
        self.norm = nn.GroupNorm(gcd(ng, n_hidden), n_hidden)
        self.linear = Linear(n_hidden, n_hidden, norm=norm, ng=ng, act=False)
        self.relu = nn.ReLU(inplace=True)
    

    def forward(self, marks, mark_idcs):

        feat = self.input(marks)
        res = feat
        ...
        output = []
        for i in range(len(mark_idcs)):
            query = q[mark_idcs[i]]
            key = k[mark_idcs[i]]
            value = v[mark_idcs[i]]

            alpha = torch.matmul(query, key.transpose(-2, -1))  
...
class Mr2A(nn.Module):
    """
    fuses marking information form marking nodes to actor nodes
    """

    def __init__(self, config) -> None:
        super(Mr2A, self).__init__()
        ng = 1
        norm = "GN"
        dropout=0.1
        n_actor = config['n_actor']
        n_mark =  config['n_mark']
        n_hidden = config['n_actor']

        self.query = nn.Linear(n_actor, n_hidden)
        self.key = nn.Linear(n_mark, n_hidden)
        self.value = nn.Linear(n_mark, n_hidden)
        self.attn_drop = nn.Dropout(dropout)
        self.norm = nn.GroupNorm(gcd(ng, n_hidden), n_hidden)
        self.linear = Linear(n_hidden, n_hidden, norm=norm, ng=ng, act=False)
        self.relu = nn.ReLU(inplace=True)
    
    def forward(self, actors, actor_idcs, marks, mark_idcs) -> Tensor:
...
class GreatNet(nn.Module):
    def __init__(self,config) -> None:
        ...
        self.mark_net = Mr2Mr(config)

        self.a2m = A2M(config)
        self.m2m = M2M(config)
        self.m2a = M2A(config)
        self.mr2a = Mr2A(config)
        self.a2a = A2A(config)
    ...
    def forward(self, data: Dict) -> Tensor:
        ...
        marks, mark_idcs = mark_gather(data['marks'])
        marks = gpu(marks)
        mark_idcs = gpu(mark_idcs)

        marks = self.mark_net(marks,mark_idcs)
        ...
        actors = self.mr2a(actors, actor_idcs, marks, mark_idcs)
