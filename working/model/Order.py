...
class Order(nn.Module):
    ...
    def forward(self, actors_, actor_idcs):
        ...
        for i in range(batch_size):
            score = order_score[actor_idcs[i]]
            matrix = score - score.T
            matrix.diagonal().zero_()
            matrix = torch.lt(matrix, 0)
            indx = torch.nonzero(matrix) + count
            order_indx.append(indx)
            count += len(actor_idcs[i])

        order_indx = torch.concatenate(order_indx, 0)
        hi = order_indx[:, 0]
        wi = order_indx[:, 1]
...
class GreatNet(nn.Module):
    ...
    def forward(self, data: Dict) -> Tensor:
        ...
        actors = self.order(actors, actor_idcs)
