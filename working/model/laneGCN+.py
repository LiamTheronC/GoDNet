...
class Mr2Mr(nn.Module):
    ...
    def __init__(self, config) -> None:
        ...
        self.query = nn.Linear(n_in, n_hidden)
        self.key = nn.Linear(n_in, n_hidden)
        self.value = nn.Linear(n_in, n_hidden)
        self.attn_drop = nn.Dropout(dropout)
        self.norm = nn.GroupNorm(gcd(ng, n_hidden), n_hidden)
        self.relu = nn.ReLU(inplace=True)
    
    def forward(self, mark, mark_idcs):

        query = self.query(mark)  
        key = self.key(mark)  
        value = self.value(mark)

        output = []
        for i in range(len(mark_idcs)):
            query = query[mark_idcs[i]]
            key = key[mark_idcs[i]]
            value = value[mark_idcs[i]]
...
class Mr2A(nn.Module):
    ...
    def forward(self, actors, actor_idcs, marks, mark_idcs) -> Tensor:
        ...
        query = self.query(actors)
        key = self.key(marks)  
        value = self.value(marks)

        output = []
        batch_size = len(actor_idcs)
        for i in range(batch_size):
            query = query[actor_idcs[i]]
            key = key[mark_idcs[i]]
            value = value[mark_idcs[i]]
