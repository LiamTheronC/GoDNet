...
from utils import to_long, gpu, rotation2
...
class GreatNet(nn.Module):
    def __init__(self,config) -> None:
        ...
        self.out_net = Linear(config["n_actor"], 160, act=False)
...
#         self.MSE = nn.MSELoss()
#         self.smL1 = nn.SmoothL1Loss(reduction = 'mean')
    
#     def forward(self, outputs: Tensor, data: Dict):
...
#         prediction = outputs[has_pred]
#         target = gt_pred[has_pred][:,:2]

#         loss = self.smL1(prediction, target)
...
class PostProcess(nn.Module):
    ...
    def forward(self, outputs, data):
        ...
        outputs = outputs.view(outputs.size(0),-1,2).cpu().detach()
        ctrs = pre_gather(data['ctrs']).cpu().detach()
        ctrs = ctrs[:,:2].unsqueeze(1)

        outputs = outputs.cumsum(dim=1) + ctrs

        has_pred = pre_gather(data['has_preds']).cpu().detach()
        gt_pred = pre_gather(data['gt2_preds']).cpu().detach()

        post_out['outputs'] = outputs
        post_out['gt_pred'] = gt_pred
        post_out['has_pred'] = has_pred

        return post_out
    
    def append(self, metrics, post_out, loss):

        outputs = post_out['outputs'] 
        gt_pred = post_out['gt_pred'] 
        has_pred = post_out['has_pred']

        #fde, this is not fde
        fde_h = has_pred[:,-1]
        fde_o = outputs[:,-1].squeeze(1)
        fde_g = gt_pred[:,-1,:2].squeeze(1)

        fde = np.sqrt(((fde_o[fde_h] - fde_g[fde_h])**2).sum(1))
        fde = fde.mean()

        ade_o = outputs[has_pred]
        ade_g = gt_pred[has_pred][:,:2]
        ade = np.sqrt(((ade_o - ade_g)**2).sum(1))
        ade = ade.mean()

        metrics['loss'].append(loss.item())
        metrics['fde'].append(fde.item())
        metrics['ade'].append(ade.item())

        return metrics    


    
    def display(self, metrics, dt, epoch,num_epochs, str='Train'):

        mean_loss = sum(metrics['loss'])/len(metrics['loss'])
        mean_fde = sum(metrics['fde'])/len(metrics['fde'])
        mean_ade = sum(metrics['ade'])/len(metrics['ade'])

        if str == 'Train':
            msg1 = '*** ({}), Epoch [{}/{}], Time: {:.1f}s ***'.format(str, epoch+1, num_epochs, dt)
        elif str == 'Validation':
            msg1 = '--- ({}), Time: {:.1f}s ---'.format(str, dt)
        elif str == 'Plot':
            msg1 = '*** {} ***'.format(str)
        
        msg2 = 'loss: {:.4f}, fde: {:.4f}, ade: {:.4f}'.format(mean_loss,mean_fde,mean_ade)
    ...
    def plot_result(self, post_out, data):
